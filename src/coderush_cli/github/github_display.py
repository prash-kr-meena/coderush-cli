...
from coderush_cli.utils import load_config
...
def display_github_metrics(metrics):
  ...
  merge_rate = (
    (total_prs_merged / total_prs_created * 100) if total_prs_created > 0 else 0
  )
  main_rate = (
    (total_prs_to_main / total_prs_merged * 100) if total_prs_merged > 0 else 0
  )
  ...
  if mode == "organization" and metrics.teams:
    ...
    console.print(
      Panel(
        "\n".join(
          f"[bold]{team}:[/] {prs} PRs, {reviews} Reviews"
  ...
  review_health = (
    "🟢" if avg_review_time < 4 else "🟡" if avg_review_time < 24 else "🔴"
  )
  ...
  change_indicator = (
    "🟢" if avg_changes < 200 else "🟡" if avg_changes < 500 else "🔴"
  )
  ...
  avg_response = (
    statistics.mean(bottleneck.review_response_times)
    if bottleneck.review_response_times
    else 0
  )
