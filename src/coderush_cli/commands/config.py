...
def config():
  ...
  try:
    ...
    if has_token:
      ...
      if Confirm.ask("Would you like to reconfigure GitHub authentication?", default=False):
        clear_user_token()
    ...
    if github_mode == "organization":
      ...
      if not github_client._check_app_installation(org_name):
        console.print("\n[yellow]Coderush GitHub App needs to be installed[/]")
        console.print("Please install the app:")
        console.print(f"1. Visit: {CODERUSH_APP['APP_URL']}")
        ...
        if not github_client._check_app_installation(org_name):
          ...
          console.print("[yellow]Note: If you just installed the app, it might take a few moments to be available.[/]")
          return
    ...
    if github_mode == "organization":
      console.print(f"[green]✓ GitHub Organization: {config_data['GITHUB_ORG']}[/]")
...
def handle_sensitive_config(config_data, name, key, prompt_text):
  ...
  if has_key:
    ...
    if Confirm.ask(f"Would you like to configure {name} integration?", default=False):
      value = Prompt.ask(prompt_text)
      if value:
        config_data[key] = value
